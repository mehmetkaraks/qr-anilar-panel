...
console.log('\x1b[32m', 'Equation Admin Template v3.0.0');
 ...
 * */
 ...
 gulp.task('cm-light-rtl:structure', function() {
	console.log('Command executed successfully compiling SCSS in assets.');
	 return gulp.src('../html/rtl/scss/layouts/collapsible-menu/light/**/*.scss') // Gets all files ending with .scss in ltr/scss and children dirs
		.pipe(sourcemaps.init())
	    .pipe(sass().on('error', sass.logError))
		// .pipe(rename('structure.css'))
		.pipe(sourcemaps.write())
	    .pipe(gulp.dest('../html/rtl/layouts/collapsible-menu/css/light/'))
	    .pipe(browserSync.reload({
	      stream: true
	}))
})
 ...
 * */
 ...
 gulp.task('cm-dark-rtl:structure', function() {
	console.log('Command executed successfully compiling SCSS in assets.');
	 return gulp.src('../html/rtl/scss/layouts/collapsible-menu/dark/**/*.scss') // Gets all files ending with .scss in ltr/scss and children dirs
		.pipe(sourcemaps.init())
	    .pipe(sass().on('error', sass.logError))
		// .pipe(rename('structure.css'))
		.pipe(sourcemaps.write())
	    .pipe(gulp.dest('../html/rtl/layouts/collapsible-menu/css/dark/'))
	    .pipe(browserSync.reload({
	      stream: true
	}))
})
...
gulp.task('watch:hdm',  gulp.series(function() {

	console.log('Executing Watch Function');

	browserSync.init({
	    server: '../html/',
		startPath: "/horizontal-dark-menu/index.html",
	})

	// Watch : - HTML

	// var html = gulp.watch('../html/horizontal-dark-menu');
	// html.on('change', function(path, stats) {
	//     browserSync.notify("Compiling, please wait!");
	//     browserSync.reload("*.html");
	// })
